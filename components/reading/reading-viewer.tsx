...
export function ReadingViewer({ book, onBack }: Props) {
    ...
    const handleReadingSurfaceClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (!isPdf && currentChapter && !isScrollMode) {
            ...
            const w = rect.width;
            if (isSimulatedMode) {
                // In simulated mode, tap left/right triggers animated flip
                if (x < w * 0.3) {
    ...
    const handleSimTouchEnd = () => {
        ...
        const containerWidth = scrollRef.current?.clientWidth || 300;
                ...
                {isPdf ? (
                    ...
                    <>
                        {isScrollMode ? (
                                ...
                                <div className="reading-page-surface">
                                    <div className="reading-page-content">
                                        {currentChapter.paragraphs.map((para, idx) => {
                                            const chapterAnnotations = annotations.filter(a => a.chapterIndex === chapterIndex && a.paragraphIndex === idx);
                                            return (
                                                <div key={idx}>
                                                    <p className="reading-line reading-line-indent">{para}</p>
                                                    {chapterAnnotations.map(annotation => (
                                                        <div key={annotation.id} className="reading-annotation">
                            ...
                            >
                                {/* Previous page (visible underneath during forward flip) */}
                                {txtPage > 0 && (
                                    <div className="reading-sim-page reading-sim-page--under">
                                        <div className="reading-page-surface">
                                            {txtPagesReadyForCurrentChapter ? renderTxtPage(txtPage - 1) : null}
                                        </div>
                                    </div>
                                )}
                                {/* Next page (visible underneath during backward flip) */}
                                {txtPage < txtTotalPages - 1 && (
                                    <div className="reading-sim-page reading-sim-page--next-under">
                                        <div className="reading-page-surface">
                                            {txtPagesReadyForCurrentChapter ? renderTxtPage(txtPage + 1) : null}
                                        </div>
                                    </div>
                                )}
                                {/* Current page with curl transform */}
                                <div
                                    className={`reading-sim-page reading-sim-page--current ${simAnimating ? `reading-sim-page--anim-${simAnimating}` : ''}`}
                                    style={simDragX !== null && !simAnimating ? {
                                        transform: `translateX(${simDragX}px) rotateY(${simDragX * -0.04}deg)`,
                                        transition: 'none',
                                    } : undefined}
                                >
                                    <div className="reading-page-surface">
                                    ...
                                    {simDragX !== null && simDragX < 0 && (
                                        <div className="reading-sim-shadow reading-sim-shadow--right" style={{ opacity: Math.min(0.4, Math.abs(simDragX) / 600) }} />
                                    )}
                                    {simDragX !== null && simDragX > 0 && (
                                        <div className="reading-sim-shadow reading-sim-shadow--left" style={{ opacity: Math.min(0.4, Math.abs(simDragX) / 600) }} />
                                    )}
                                </div>
                            </div>
                        ) : (
